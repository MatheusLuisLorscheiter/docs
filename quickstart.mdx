...
const response = await fetch('https://{url-key}/api/campaigns', {
  ...
  body: JSON.stringify(audioCampaign)
});

const result = await response.json();
console.log('Campanha de áudio criada:', result);
```
...
result = response.json()
print('Campanha de áudio criada:', result)
```
...
curl_setopt_array($curl, [
    ...
    CURLOPT_POST => true,
    CURLOPT_HTTPHEADER => $headers,
    CURLOPT_POSTFIELDS => json_encode($audio_campaign)
]);
