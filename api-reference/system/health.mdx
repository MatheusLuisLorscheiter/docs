...
<Note>
O endpoint de health check fornece informações sobre o status de todos os componentes da aplicação, incluindo banco de dados, Redis e outros serviços.
</Note>
...
### Redis
- Verifica conexão com servidor Redis
- Tenta executar comando PING
...
async function monitorHealth() {
  ...
  try {
    ...
    if (health.status !== 'UP') {
      ...
      console.error('⚠️ API com problemas!');
    }
  } catch (error) {
