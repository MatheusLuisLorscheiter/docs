...
|-------|------|-------------|-----------|
| name | string | Sim | Nome da campanha entre 2 e 100 caracteres |
| evolutionUrl | string | Sim | URL completa da sua Evolution API |
| evolutionApiKey | string | Sim | API Key da sua Evolution API |
| evolutionInstance | string | Sim | Nome da instância do WhatsApp |
| contacts | array | Sim | Lista de contatos que receberão a mensagem |
| contacts[].phoneNumber | string | Sim | Número de telefone no formato 5511999999999 |
| contacts[].name | string | Não | Nome do contato para variáveis |
| message | string | Sim | Conteúdo da mensagem com suporte a variáveis |
| mediaUrl | string | Não | URL do arquivo de mídia |
| mediaType | string | Não | Tipo de mídia: image, video, audio, document |
| caption | string | Não | Legenda para mídia |
| scheduledFor | datetime | Não | Data e hora para agendamento |
...
| message | string | Mensagem de erro ou null em caso de sucesso |
...
response = requests.post(
    'https://api.disparador.com/api/campaigns',
    headers=headers,
    json=data
)
...
1. Consultar Status - Acompanhe o progresso
2. Listar Campanhas - Veja todas as campanhas  
3. Gerenciar Campanha - Pause, retome ou cancele