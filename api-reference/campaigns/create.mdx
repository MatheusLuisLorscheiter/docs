...
<ParamField body="name" type="string" required={true}>
  Nome da campanha. Deve ter entre 2 e 100 caracteres.
</ParamField>

<ParamField body="evolutionUrl" type="string" required={true}>
  URL completa da sua Evolution API (ex: https://sua-evolution-api.com).
</ParamField>

<ParamField body="evolutionApiKey" type="string" required={true}>
  API Key da sua Evolution API.
</ParamField>

<ParamField body="evolutionInstance" type="string" required={true}>
  Nome da instância do WhatsApp conectada na Evolution API.
</ParamField>

<ParamField body="contacts" type="array" required={true}>
  Lista de contatos que receberão a mensagem.
</ParamField>

<ParamField body="contacts[].phoneNumber" type="string" required={true}>
  Número de telefone do contato (formato: 5511999999999).
</ParamField>

<ParamField body="contacts[].name" type="string" required={false}>
  Nome do contato (usado para variáveis na mensagem).
</ParamField>

<ParamField body="message" type="string" required={true}>
  Conteúdo da mensagem. Suporta variáveis como {{name}}.
</ParamField>

<ParamField body="mediaUrl" type="string" required={false}>
  URL do arquivo de mídia (imagem, vídeo, áudio, documento).
</ParamField>

<ParamField body="mediaType" type="string" required={false}>
  Tipo de mídia: image, video, audio, document.
</ParamField>

<ParamField body="caption" type="string" required={false}>
  Legenda para mídia (quando mediaUrl for fornecido).
</ParamField>

<ParamField body="scheduledFor" type="datetime" required={false}>
  Data e hora para agendamento (formato: YYYY-MM-DDTHH:mm:ss).
</ParamField>
