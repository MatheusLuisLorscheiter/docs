...
<Tip>
Retorna uma lista de todas as campanhas da empresa autenticada, ordenadas por data de criação.
</Tip>
...
| Código | Descrição |
|--------|-----------|
| 400 | Access token inválido ou não fornecido |
| 401 | Token de acesso não autorizado |
...
<Note>
A API retorna todas as campanhas da empresa em ordem cronológica (mais recentes primeiro).
</Note>

**Funcionalidades futuras planejadas:**

<CardGroup cols={2}>
  <Card title="Filtros Avançados" icon="filter" color="#8b5cf6">
    Filtro por status, data de criação e nome
  </Card>
  <Card title="Controles de Lista" icon="settings" color="#f59e0b">
    Paginação e ordenação personalizada
  </Card>
</CardGroup>
...
// Função para monitorar campanhas ativas
async function monitorarCampanhas() {
  ...
  activeCampaigns.forEach(campaign => {
    ...
    console.log(`${campaign.name}: ${progress}% concluída`);
  });
}
...
def gerar_relatorio_campanhas():
    ...
    print(f"Relatório de Campanhas - {datetime.now().strftime('%d/%m/%Y %H:%M')}")
    print("-" * 50)
