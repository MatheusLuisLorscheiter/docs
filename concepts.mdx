...
<Warning>
Certifique-se de que sua Evolution API está configurada e funcionando antes de usar a API Disparador. Para mais detalhes, consulte a [documentação oficial da Evolution API](https://docs.evolution-api.com/).
</Warning>
...
```json
{
  "message": "Olá {{name}}! Sua compra de {{product}} foi confirmada.",
  "contacts": [
    {
...
<CardGroup cols={2}>
  ...
  <Card title="{{phone}}" icon="phone" color="#2563eb">
    Número de telefone
  </Card>
  <Card title="{{company}}" icon="building" color="#7c3aed">
    Nome da empresa
  </Card>
  <Card title="{{date}}" icon="calendar" color="#ea580c">
    Data atual
  </Card>
</CardGroup>
...
```json
{
  "message": "Olá {{name}}! Bem-vindo à {{company}}. Hoje é {{date}}.",
  "contacts": [
    {
      "phoneNumber": "5511999999999",
      "name": "João Silva"
